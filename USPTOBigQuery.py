...
class PatentBigQuery:
    ...
    def get_2005_grant_cpc(self, args_array):
        ...
        if os.path.isfile(args_array['CPC_2005_dataframe_filename']):
            print("-- Found existing 2005 grant CPC class data file...")
            # Read the dataframe back from file
            df = pd.read_pickle(args_array['CPC_2005_dataframe_filename'])
            # Conver to JSON object
            json_obj = df.to_json(orient='records')

            print("-- All 2005 CPC codes retreived from Google BigQuery successfully.")
            logger.info("-- All 2005 CPC codes retreived from Google BigQuery successfully.")
        ...
        # If file is not found then get from BigQuery
            ...
            try:
                ...
                sql = """
                SELECT
                publication_number, cpc, grant_date
                ...
                bq.pickle_dataframe_response(df, args_array['CPC_2005_dataframe_filename'])
                print("-- 2005 grant CPC class stored to file " + args_array['CPC_2005_dataframe_filename'] + "...")
                # Conver to JSON object
                json_obj = df.to_json(orient='records')
                ...
                print("-- Failed to retrieve 2005 CPC codes from Google BigQuery")
                logger.info("-- Failed to retrieve all 2005 CPC codes from Google BigQuery")
                exit(1)

    # Inserts the CPC classifications for all 2005 patents
    # into the database
    def insert_2005_grant_cpc(self, args_array, json_obj):
        ...
        try:
            ...
            print("-- All 2005 CPC codes inserted into database successfully")
            logger.info("-- All 2005 CPC codes inserted into database successfully")
            exit(0)
            ...
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            logger.error("Exception: " + str(exc_type) + " in Filename: " + str(fname) + " on Line: " + str(exc_tb.tb_lineno) + " Traceback: " + traceback.format_exc())
            print("-- Failed to insert all 2005 CPC codes inserted into database")
            logger.info("-- Failed to insert all 2005 CPC codes inserted into database")
            exit(1)
